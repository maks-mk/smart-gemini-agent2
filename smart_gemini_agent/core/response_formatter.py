...
class ResponseFormatter:
    ...
    def improve_file_content_formatting(self, response: str) -> str:
        ...
        if "Содержимое текущей рабочей директории" in response or re.search(r'\[(FILE|DIR)\]', response):
    ...
    def _handle_json_response(self, response: str) -> Optional[str]:
        ...
        try:
            ...
            if isinstance(json_data, dict) and 'file_path' in json_data and 'content' in json_data:
                filename = json_data['file_path']
                content = json_data['content']
                language = json_data.get('language', self.file_formatter.get_language_by_filename(filename))
                
                # Автоматическое форматирование содержимого
                content = self.file_formatter.format_content(content, language, filename)
                
                formatted_response = f"Содержимое файла `{filename}`:\n\n```{language}\n{content}\n```"
                
                if self.debug_mode:
                    logger.debug(f"Успешно отформатирован ответ-JSON для файла {filename}")
    ...
    def _format_list_directory_response(self, response: str) -> Optional[str]:
        ...
        try:
            ...
            items = re.findall(r'(\[(FILE|DIR)\])\s*([^\s\[\]]+)', content_part)
        ...
        try:
            ...
            if isinstance(parsed, list) and len(parsed) >= 2:
                ...
                if filename_match:
                    ...
                    # Автоматическое форматирование содержимого
                    actual_content = self.file_formatter.format_content(actual_content, language, filename)
