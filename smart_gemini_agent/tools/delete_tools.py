    ...
    def _run(self, file_path: str) -> str:
        ...
        try:
            ...
            if target_path.is_dir():
                return self._format_error(f"{file_path} является директорией. Используйте инструмент для удаления директорий.")
            ...
            return self._format_error(f"Не удалось удалить файл {file_path}. Причина: {str(e)}")
    ...
    def _run(self, dir_path: str, recursive: bool = False) -> str:
        ...
        try:
            ...
            if not target_path.is_dir():
                return self._format_error(f"{dir_path} является файлом. Используйте инструмент для удаления файлов.")
            
            # Удаляем директорию
            if recursive:
                shutil.rmtree(target_path)
                return self._format_success(f"Директория {dir_path} и все её содержимое успешно удалены")
            else:
                target_path.rmdir()  # Удаляет только пустую директорию
                return self._format_success(f"Пустая директория {dir_path} успешно удалена")
            
        except OSError as e:
            if "Directory not empty" in str(e):
                return self._format_error(f"Директория {dir_path} не пуста. Используйте recursive=true для рекурсивного удаления.")
            return self._format_error(f"Не удалось удалить директорию {dir_path}. Причина: {str(e)}")
        except Exception as e:
            return self._format_error(f"Не удалось удалить директорию {dir_path}. Причина: {str(e)}")
