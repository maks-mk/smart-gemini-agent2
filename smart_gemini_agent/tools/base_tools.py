...
class SafeToolMixin(ABC):
    ...
    def _validate_path(self, file_path: str) -> Tuple[bool, Optional[str], Optional[Path]]:
        """
        ...
        try:
            ...
            if not str(target_path).startswith(str(self.working_directory)):
                return False, f"Попытка доступа к файлу вне рабочей директории: {target_path}", None
...
class FileSystemTool(BaseTool, SafeToolMixin):
    ...
    def __init__(self, working_directory: Union[str, Path], **kwargs):
        ...
        SafeToolMixin.__init__(self, working_directory, **kwargs)
