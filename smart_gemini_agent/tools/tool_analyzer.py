"""
Анализатор и категоризатор инструментов
"""
...
class ToolAnalyzer:
    ...
    def analyze_tools(self, tools: List[BaseTool]) -> Dict[str, List[BaseTool]]:
        ...
        if len(self.tools_map['other']) > len(tools) * 0.3:
            logger.warning(f"Много инструментов в категории 'other' ({len(self.tools_map['other'])}). "
